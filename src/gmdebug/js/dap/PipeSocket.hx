...
class PipeSocket {
		...
		return new Promise(function(success, failure) {
			...
			haxe.Timer.delay(() -> {
				...
				for (socket in socketsStatus) {
					Fs.appendFileSync(HxPath.join(["log.txt"]),
						'getValidSocket/delayValidConnection iterate socket $socket');
			...
			server.on('connection', (socket:Socket) -> {
				...
				var invalidateSocket = () -> {
					Fs.appendFileSync(HxPath.join(["log.txt"]),
						'getValidSocket/onConnection/invalidSocket $details socket $socket lost!');
	...
	@:async function aquireWindowsSocket(serverIn:js.node.net.Server,
			serverOut:js.node.net.Server):{sockIn:Socket, sockOut:Socket} {
		trace("AquireWindowsSock");
		final socks = @:await Promise.inParallel([getValidSocket(serverIn
			, "In"), getValidSocket(serverOut, "Out")]);
