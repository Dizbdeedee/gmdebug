...
import js.node.Fs;
import gmdebug.js.dap.LuaDebugger.Programs;
...
class InitBundle {
	...
	function new(req:Request<Dynamic>, args:GmDebugLaunchRequestArguments, luadebug:LuaDebugger) {
		...
		if (!args.nodebugClient && args.clientFolder == null) {
			req.composeFail(DEBUGGER_INVALID_CLIENTFOLDER_UNSPECIFIED)
				.send(luadebug);
		...
		if (args.copyAddonBaseFolder != null && HxPath.isAbsolute(args.copyAddonBaseFolder)) {
			req.composeFail(DEBUGGER_INVALID_COPYFOLDER_ABSOLUTE)
				.send(luadebug);
	...
	public static function initBundle(req:Request<Dynamic>, args:GmDebugLaunchRequestArguments,
			luadebug:LuaDebugger):Outcome<InitBundle, InitBundleException> {
		return try {
	...
	function validateProgramPath(programPath:String):InitError {
		...
		} else if (!Fs.statSync(programPath)
			.isFile()) {
	...
	function validateServerFolder(serverFolder:String):InitError {
		return if (serverFolder == null) {
			...
			} else if (!Fs.statSync(serverFolder)
				.isDirectory()) {
				Error(DEBUGGER_INVALID_SERVERFOLDER_NOTDIR);
			} else if (!Fs.existsSync(addonFolder) || !Fs.statSync(addonFolder)
				.isDirectory()) {
	...
	function validateClientFolder(folder:String):InitError {
		...
		} else if (!Fs.statSync(folder)
			.isDirectory()) {
			Error(DEBUGGER_INVALID_CLIENTFOLDER_NOTDIR, {folder: folder});
		} else if (!Fs.existsSync
			(addonFolder) || !Fs.statSync(addonFolder)
			.isDirectory()) { // BY DEFAULT ADDONS FOLDER IS NOT CREATED (slap)
...
@:using(gmdebug.js.dap.InitBundle.InitErrorUsing)
