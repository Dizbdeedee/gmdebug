...
import gmdebug.js.dap.EventIntercepter;
import gmdebug.protocol.ext.paths.GMDNormalAbsPath;
import js.node.Timers;
import gmdebug.js.dap.clients.ClientStorage;
import gmdebug.js.dap.io.ClientStorageWithHandshake;
...
import gmdebug.js.dap.ResponseIntercepter;
import gmdebug.js.dap.GmodClientOpener;
import js.node.stream.Readable;
import js.node.stream.Writable;
import gmdebug.js.dap.LaunchProcessor;
import gmdebug.js.dap.FileWatcher;
import gmdebug.js.dap.Log;
import gmdebug.js.dap.FileLookup;
import gmdebug.js.dap.OutputFilterer;
import gmdebug.js.dap.FileTracker;
import gmdebug.js.dap.BreakpointRequester;
...
@:keep @:await class LuaDebugger extends DebugSession {
	...
	function childProcessSetup(childProcess:ChildProcess) {
		childProcess.on("error", (err) -> {
			new ComposedEvent(output, {category: Stderr,
				output: err.message + "\n" + err.stack,
				data: null
			}).send(this);
	...
	function setupPlayer(clientID:Int) {
		clients.sendClient(clientID,
			new ComposedGmDebugMessage(intialInfo, {location: initBundle.serverFolder, dapMode: Launch}));
		clients.sendClient(clientID,
			new ComposedGmDebugMessage(GmMsgType.clientID, {id: clientID}));
		prevRequests.get(setBreakpoints)
			.run((msg) -> clients.sendClient(clientID, msg));
		prevRequests.get(setExceptionBreakpoints)
			.run((msg) -> clients.sendClient(clientID, msg));
		prevRequests.get(setFunctionBreakpoints)
			.run((msg) -> clients.sendClient(clientID, msg));
	...
	function playerRemovedMessage(x:GMPlayerRemovedMessage) {
		new ComposedEvent(thread, {
			threadId: clients.getByGmodID(x.playerID)
				.clID,
			reason: Exited
		}).send(this);
	}
	...
	@:async function pokeServerTimeout() {
		...
		switch (dapMode) {
			case ATTACH:
				clients.sendServer(new ComposedGmDebugMessage(intialInfo
					, {location: initBundle.serverFolder, dapMode: Attach}));
			case LAUNCH(_):
				clients.sendServer(new ComposedGmDebugMessage(intialInfo
		...
		switch (message.type) {
			case Request:
				...
				try {
					...
					trace(e.stack);
					beginShutdown(MESSAGE_INTERP_FAILURE(message, new DoNotPrintStr(e.toString())));
