...
class RedirectWorker {
	...
	public static function makeChildProcess(program:String,
			args:Array<String>):js.node.child_process.ChildProcess {
	...
	static function HandleCommandLineDisplay(r:Redirector, screenSize:Int) {
		...
		if (cmdLine.length > 0
			&& !isAllWhitespace(cmdLine)
			&& oldCmdLine != null
			&& !cmdLine.startsWith(oldCmdLine)) {
	...
	public static function main() {
		...
		Redirector.K32.SetConsoleCtrlHandler(Callback.call_(rtypes.bool, [rtypes.ulong], handleDeth)
			, cast true);
		...
		function loop() {
			...
			for (i in 0...screenSize - 2) {
				if (i * CON_LINE_LENGTH >= output.length)
					break;
