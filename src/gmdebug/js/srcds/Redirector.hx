...
import gmdebug.js.srcds.BufferCompat as Buffer;
...
private extern class Kernel32 {
	...
	function CreateProcessA(lpApplicationName:Buffer, lpCommandLine:Buffer,
		lpProcessAttributes:SecurityInfo, lpThreadAttributes:SecurityInfo, bInheritHandles:BufBool,
		dwCreationFlags:BufInt, lpEnvironment:Buffer, lpCurrentDirectory:Buffer, lpStartupInfo:Buffer,
		lpProcessInformation:Buffer):Bool;
...
class Redirector {
	...
	public static final K32:Kernel32 = cast new Library("kernel32", {
		untyped {
			...
			MapViewOfFile: [refType
				(rtypes.void), [refType(rtypes.void), DWORD, DWORD, DWORD, rtypes.size_t]],
			...
			CreateProcessA: [
				...
				[
					refType
					(rtypes.char), refType(rtypes.char), refType(SECURITY_ATTR), refType(SECURITY_ATTR),
					rtypes.bool, rtypes.ulong, refType
					(rtypes.void), refType(rtypes.char), refType(STARTUP_INFO),
	...
	public function Start(program:String, args:Array<String>) {
		...
		var result = K32.CreateProcessA(null, command, null, null, true, 16, null, null, si.ref()
			, processInfo.ref());
		if (!result)
			throw "Good luck debugging this, asshole";
	}
