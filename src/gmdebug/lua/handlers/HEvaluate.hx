...
class HEvaluate implements IHandler<EvaluateRequest> {
	...
	public function handle(evalReq:EvaluateRequest):HandlerResponse {
		...
		if (args.context == Hover) {
			expr = NativeStringTools.gsub(expr, ":"
				, "."); // a function call is probably not intended from a hover.
		}
		// trace('expr : $expr');
		final resp:ComposedProtocolMessage = switch (Util.compileString(expr, "GmDebug")) {
			...
			case Success(func):
				if (fid != null) {
					...
					final eval = createEvalEnvironment(fid.getValue()
						.actualFrame + offsetHeight);
