...
class DebugContext {
	...
	**/
	...
	public static inline function getHeight(?infos:haxe.PosInfos) {
		if (debugContextState == NotDebug)
			trace("Attempt to get height not in debug context??");
	...
	public static function mapHeight(id:Int):Int {
		for (sh in 2...9999) {
			...
			if (info == null)
				break;
			var compareSource = DebugLib.getinfo(1, "S")
				.source;
			trace('${info.source} $compareSource');
			if (info.source == null || Util.isCSource(info.source) || info.source == compareSource) {
				continue;
			}
			mappedHeights[id] = sh;
