...
class PipeSocketAsync implements DebugIO {
	public var input:PipeInputAsync;
	...
	public function new(_locs:PipeLocations) {
		trace('NEW ASYNC PIPE SOCKET :D ${_locs.folder}');
		locs = _locs;
...
class PipeInputAsync extends Input {
	var file:File;
	final locs:PipeLocations;
...
class PipeOutputAsync extends Output {
	var file:File;
	final locs:PipeLocations;
