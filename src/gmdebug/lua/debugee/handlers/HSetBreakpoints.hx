...
import gmdebug.lua.debugee.managers.BreakpointManager.Breakpoint as GmDebugBP;
...
class HSetBreakpoints implements IHandler<SetBreakpointsRequest> {
	...
	public function handle(req:SetBreakpointsRequest):HandlerResponse {
		...
		final json = tink.Json.stringify
			((cast resp : SetBreakpointsResponse)); // in pratical terms they're the same
		debugee.send(json);
		return WAIT;
	}
}
