...
class ClientStorageDef implements ClientStorage {
	...
	function connectionProcess(slotLocation:String,
			slots:Array<SlotStatus>):FutureArray<Outcome<PipeSocket, Error>> {
	...
	function aquireClients(slotLocation:String, slots:Array<SlotStatus>):Future<Array<Client>> {
		return connectionProcess(slotLocation, slots).inSequence()
			.map((clientOutcomesArr) -> {
				var clientsAquired = [];
	...
	function continueAquireServer(serverLoc:String, timeout:Int):Promise<Server> {
		...
		function nextConnection(results:Array<Outcome<PipeSocket, Error>>):Promise<Server> {
				...
				switch (result) {
					case Success(pipeSocket):
						if (chosenServer != null)
							return Promise.reject(new Error("More than one server =("));
			...
			trace("Running again.");
			return connectionProcess(serverLoc, serverSlots).inSequence()
				.next(nextConnection);
		}
		return connectionProcess(serverLoc, serverSlots).inSequence()
			.next(nextConnection);
