	...
	processBytes(rawBytes,clientNo) {
		...
		try {
			...
			if(((_g2) instanceof haxe_io_Eof)) {
				...
				this.prevClientResults[clientNo] = null;
				this.prevBytes[clientNo] = rawBytes.sub(this.lastGoodPos,rawBytes.length - this.lastGoodPos);
				console.log("src/gmdebug/dap/BytesProcessor.hx:46:",e);
				return [];
			} else {
...
class gmdebug_dap_LaunchProcess {
	constructor(programPath,luaDebug,programArgs) {
		...
		this.childProcess.on("error",function(err) {
			...
			console.log("src/gmdebug/composer/ComposedEvent.hx:32:","sending from dap " + _this.event);
			luaDebug.sendEvent(_this);
			console.log("src/gmdebug/dap/LaunchProcess.hx:44:","Child process error///");
			console.log("src/gmdebug/dap/LaunchProcess.hx:45:",err.message);
			console.log("src/gmdebug/dap/LaunchProcess.hx:46:",err.stack);
			console.log("src/gmdebug/dap/LaunchProcess.hx:47:","Child process error end///");
			luaDebug.shutdown();
		});
	}
	write(chunk) {
...
class gmdebug_dap_LuaDebugger extends vscode_debugAdapter_DebugSession {
	...
	checkPrograms() {
		try {
			...
			let _g1 = haxe_Exception.caught(_g);
			console.log("src/gmdebug/dap/LuaDebugger.hx:92:","Xdotool not found");
			console.log("src/gmdebug/dap/LuaDebugger.hx:93:",_g1.toString());
		}
	}
	uncaughtException(err,origin) {
		console.log("src/gmdebug/dap/LuaDebugger.hx:98:",err.message);
		console.log("src/gmdebug/dap/LuaDebugger.hx:99:",err.stack);
		this.shutdown();
	}
				...
				while(_g_current < _this.length) {
					...
					if(!_gthis.clientsTaken.h.hasOwnProperty(_g1_key)) {
						try {
							...
							console.log("src/gmdebug/dap/LuaDebugger.hx:113:","could not aquire in " + _g1_value);
						}
					}
				}
	...
	processCustomMessages(x) {
		console.log("src/gmdebug/dap/LuaDebugger.hx:165:","custom message");
		switch(x.msg) {
		case 0:
			this.playerAddedMessage(x.body).handle(function(out) {
				...
				case 0:
					if(out.data) {
						console.log("src/gmdebug/dap/LuaDebugger.hx:171:","Whater a sucess");
					} else {
						console.log("src/gmdebug/dap/LuaDebugger.hx:173:","Could not add a new player...");
					}
					break;
				case 1:
	...
	pokeServerNamedPipes(attachReq) {
		...
		return tink_core_Future.irreversible(function(__return) {
			try {
				...
				let timedout = true;
				console.log("src/gmdebug/dap/LuaDebugger.hx:193:",timeout);
				let __doCount = 0;
					...
					try {
						...
						_gthis.aquireWriteSocket(_gthis.locs.write).handle(function(__t10) {
							try {
								...
								_gthis.aquired = true;
								console.log("src/gmdebug/dap/PipeSocket.hx:51:","Aquired socket...");
								__return(tink_core_Outcome.Success(null));
								return;
							} catch( _g ) {
								let _g1 = haxe_Exception.caught(_g).unwrap();
			...
			try {
				...
				tink_core_Future.ofJsPromise(open(inp,js_node_Fs.constants.O_RDWR | js_node_Fs.constants.O_NONBLOCK)).handle(function(__t12) {
					try {
						...
						let fd = __t12_result;
						console.log("src/gmdebug/dap/PipeSocket.hx:73:",fd);
...
class gmdebug_dap_RequestRouter {
	...
	h_variables(req) {
		...
		if(ref <= 0) {
			console.log("src/gmdebug/dap/RequestRouter.hx:97:","invalid variable reference");
	...
	generateInitFiles(serverFolder) {
		...
		console.log("src/gmdebug/dap/RequestRouter.hx:221:",process.cwd());
		console.log("src/gmdebug/dap/RequestRouter.hx:222:",process.cwd());
		console.log("src/gmdebug/dap/RequestRouter.hx:223:",appendFile);
		let appendContents;
