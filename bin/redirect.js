...
class gmdebug_dap_srcds_RedirectWorker {
	...
	static handleDeth(...rest) {
		console.log("src/gmdebug/dap/srcds/RedirectWorker.hx:70:","deth handled");
		gmdebug_dap_srcds_RedirectWorker.canLoop = false;
		return false;
	}
	static main() {
		let r = new gmdebug_dap_srcds_Redirector();
		gmdebug_dap_srcds_RedirectWorker.red = r;
		console.log("src/gmdebug/dap/srcds/RedirectWorker.hx:81:",process.argv);
		r.Start(process.argv[2],process.argv.slice(3));
		let bJustStarted = false;
		gmdebug_dap_srcds_Redirector.K32.SetConsoleCtrlHandler(ffi_$napi_Callback(RefNapi.types.bool,[RefNapi.types.ulong],gmdebug_dap_srcds_RedirectWorker.handleDeth),true);
		let outputBuffer = [];
		let oldOutput = [];
		let loop = function() {
			...
			if(!r.SetScreenBufferSize(screenSize)) {
				console.log("src/gmdebug/dap/srcds/RedirectWorker.hx:92:","Failed to set screen size " + screenSize);
			}
			if(process.stdin.readable) {
				...
				while(true) {
					...
					if(read != null) {
						console.log("src/gmdebug/dap/srcds/RedirectWorker.hx:100:",read);
				...
				if(readStr_b.length > 0) {
					console.log("src/gmdebug/dap/srcds/RedirectWorker.hx:105:",readStr_b);
			...
			if(oldOutput.length > 0) {
				...
				while(_g < _g1) {
					let i = _g++;
					if(outputBuffer[outputBuffer.length - 1 - i].startsWith(oldOutput[lastLine])) {
						++firstNewLine;
						hist = true;
						let _g = i + 1;
						let _g1 = outputBuffer.length - 1;
						while(_g < _g1) if(!outputBuffer[outputBuffer.length - 1 - _g++].startsWith(oldOutput[--lastLine])) {
				...
				if(firstNewLine < 0) {
					if(hist) {
						...
						console.log("src/gmdebug/dap/srcds/RedirectWorker.hx:159:","Console moved too fast " + firstNewLine);
						firstNewLine = 0;
					}
				}
		...
		mainLoop = function() {
			...
			if(gmdebug_dap_srcds_RedirectWorker.canLoop) {
				js_node_Timers.setTimeout(mainLoop,25);
			} else {
				console.log("src/gmdebug/dap/srcds/RedirectWorker.hx:189:","Bugger off please");
				r.Destroy();
				console.log("src/gmdebug/dap/srcds/RedirectWorker.hx:191:","exiting");
				process.exit(1);
				console.log("src/gmdebug/dap/srcds/RedirectWorker.hx:193:","ohohoho");
			}
		};
		mainLoop();
		console.log("src/gmdebug/dap/srcds/RedirectWorker.hx:197:","mm");
	}
}
...
class gmdebug_dap_srcds_Redirector {
	constructor() {
		...
		console.log("src/gmdebug/dap/srcds/Redirector.hx:183:",point.address());
		...
		console.log("src/gmdebug/dap/srcds/Redirector.hx:185:",this.map_file.address());
		if(RefNapi.isNull(this.map_file)) {
			throw haxe_Exception.thrown("Could not create file mapping");
		}
		...
		if(RefNapi.isNull(this.event_parent_send)) {
			throw haxe_Exception.thrown("Could not create parent send event");
		}
		...
		if(RefNapi.isNull(this.event_child_send)) {
			throw haxe_Exception.thrown("Could not create child send event");
		}
	}
	Destroy() {
		gmdebug_dap_srcds_Redirector.K32.CloseHandle(this.map_file);
		gmdebug_dap_srcds_Redirector.K32.CloseHandle(this.event_parent_send);
		gmdebug_dap_srcds_Redirector.K32.CloseHandle(this.event_child_send);
		if(this.processInfo.dwProcessId != 0) {
			console.log("src/gmdebug/dap/srcds/Redirector.hx:205:","Attempting destruction");
			gmdebug_dap_srcds_Redirector.K32.TerminateProcess(this.processInfo.hProcess,1);
			gmdebug_dap_srcds_Redirector.lib.memset(this.processInfo.ref(),0,gmdebug_dap_srcds_Redirector.PROCESS_INFO.size);
		}
	}
	Start(program,args) {
		...
		gmdebug_dap_srcds_Redirector.lib.memset(si.ref(),0,gmdebug_dap_srcds_Redirector.STARTUP_INFO.size);
		si.cb = gmdebug_dap_srcds_Redirector.STARTUP_INFO.size;
		if(!gmdebug_dap_srcds_Redirector.K32.CreateProcessA(null,RefNapi.allocCString("" + program + " -HFILE " + this.map_file.address() + " -HPARENT " + this.event_parent_send.address() + " -HCHILD " + this.event_child_send.address() + " " + args.join(" ")),null,null,true,16,null,null,si.ref(),this.processInfo.ref())) {
	...
	ReadText(iBeginLine,iEndLine) {
		...
		if(!gmdebug_dap_srcds_Redirector.K32.SetEvent(this.event_parent_send)) {
			throw haxe_Exception.thrown("Event not set...");
		}
		if(!this.WaitForResponse()) {
			console.log("src/gmdebug/dap/srcds/Redirector.hx:248:","Could not wait...");
			throw haxe_Exception.thrown("Yikes");
		}
		let pBuf = this.GetMappedBuffer();
		let output = pBuf[0] == 1 ? RefNapi.readCString(pBuf.buffer.reinterpretUntilZeros(RefNapi.types.char.size),RefNapi.types.int.size) : "_";
		this.ReleaseMappedBuffer(pBuf);
		return output;
	}
	...
	GetMappedBuffer() {
		let pbuf2 = gmdebug_dap_srcds_Redirector.intBuf(gmdebug_dap_srcds_Redirector.K32.MapViewOfFile(this.map_file,6,RefNapi.NULL,RefNapi.NULL,0).reinterpret(3 * RefNapi.types.int.size),3);
		if(RefNapi.isNull(pbuf2.buffer)) {
			console.log("src/gmdebug/dap/srcds/Redirector.hx:284:","Wuh oh");
		}
		return pbuf2;
	}
	WriteText(input) {
		let pBuf = this.GetMappedBuffer();
		pBuf[0] = 2;
		pBuf.buffer.reinterpret((input.length + 1) * RefNapi.types.char.size,RefNapi.types.int32.size).write(input + "\x00",0);
		this.ReleaseMappedBuffer(pBuf);
		gmdebug_dap_srcds_Redirector.K32.SetEvent(this.event_parent_send);
		if(!this.WaitForResponse()) {
			throw haxe_Exception.thrown("Could not wait");
	...
	GetScreenBufferSize() {
		...
		if(pBuf1[0] == 1) {
			...
			console.log("src/gmdebug/dap/srcds/Redirector.hx:326:",pBuf1[0]);
			console.log("src/gmdebug/dap/srcds/Redirector.hx:327:",pBuf1[1]);
			bufferSize = -1;
		}
		this.ReleaseMappedBuffer(pBuf1);
		return bufferSize;
	...
	WaitForResponse() {
		...
		if(waitResult == gmdebug_dap_srcds_Redirector.WAIT_OBJECT_0 + 1) {
			console.log("src/gmdebug/dap/srcds/Redirector.hx:341:","Process ended");
			throw haxe_Exception.thrown("Ended");
		}
...
gmdebug_dap_srcds_RedirectWorker.canLoop = true;
